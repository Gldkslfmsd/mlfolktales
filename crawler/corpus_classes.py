    ...
    def pretty_write(self):
        ...
        for story in self.all_stories:
            if story.get_atu_level_1() == 'UNKNOWN':
                path = os.path.join(self.BASEDIR, 'Fairytale Corpus', story.get_language(), story.get_atu_level_1(), '')
            else:
                path = os.path.join(self.BASEDIR,'Fairytale Corpus', story.get_language(), story.get_atu_level_1(), story.get_atu_level_2(), '')
...
class AbstractFlatCorpus(Corpus):
    ...
    def pretty_write(self):
        ...
        p = os.path.join(self.BASEDIR, "stories.csv")
        df.to_csv(p)
...
class XmlFlatCorpus(AbstractFlatCorpus):
    DIR = os.path.join(Corpus.BASEDIR, "corpus_xml")
    FILE_SUFFIX = "xml"
